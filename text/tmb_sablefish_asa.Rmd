---
title: Preliminary results for an age-structured population model for sablefish (*Anoplopoma
...
The maturity curve fit to all years was the best-fitting model. 
We used a logistic regression approach in R, such that the probability $p$ of being mature at a given age on the logit scale is a linear function of age ($a$):
...
Data from the 1997-2017 ADF&G longline surveys were used to obtain proportion female by age (Figure 2C). 
These values were used to estimate spawning stock biomass within the model. 
...
kable(def_tbl, "latex", booktabs = TRUE, escape = FALSE)
```
