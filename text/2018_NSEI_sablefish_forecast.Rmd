...
Internal ADF&G review: Thursday, May 10;  

Final version approved by reviewers & managers: Friday, May 13;  
...
- The adjusted population estimate from the `r YEAR` mark-recapture study in Chatham Strait is `r abd_lyadj` individuals. This is a revised estimate and a change from the previous estimate of `r abd_ly` individuals from the `r YEAR` mark-recapture study. Had there been a marking survey in 2016, the increase in the population would have been observed a year earlier. The adjusted forecast of abundance for `r YEAR+1` is `r abd_foradj` individuals (`r fig('retro', display="cite")`);
...
- The federal stock assessment for sablefish reported a significant increase in abundance and associated quota. The recommended federal Acceptable Biological Catch for the `r YEAR+1` commercial longline sablefish fishery is `r formatC(fed$ABC_thisyr, format="d", big.mark=",")` metric tons, a `r formatC(abs(fed$perc_change), format="f", digits=1)`% `r fed$eval` from the `r YEAR` $ABC$ of `r formatC(fed$ABC_lastyr, format="d", big.mark=",")` metric tons (Hanselman et al. `r YEAR`). The $F_{ABC}$ presented in the summary table above is not directly comparable to the federal harvest rate of $F_{40\%}$, because the methods used to assess abundance and determine $F$ values are different. Moreover, the federal assessment authors have taken additional conservation measures to adjust for whale depredation and dampen the predicted population increase from the strong 2014 year class (Hanselman et al. `r YEAR`).

# Changes to the NSEI sablefish assessment for `r YEAR+1` relative to `r YEAR`


1.  As discussed in the Summary and Section I of this report, the `r YEAR` estimate of exploited abundance used to determine the $ABC$ was adjusted to account for the high degree of uncertainty in recent recruitment events. We used the 15% percentile from the posterior distribution of abundance as the input to the yield per recruit analysis and calculation of $ABC$ (`r fig('N_posterior', display = "cite")`). The relative impacts of adjusting the abundance can be seen in the Summary table at the beginning of this report and in `r fig('retro', display="cite")`.
...
The `r YEAR-1` marking survey released `r formatC(tagged, format="d", big.mark=",")` marked fish. We accounted for tags recovered outside of the NSEI or period of recapture, natural and fishing mortality, and differences in the size of fish captured in the pot survey and the longline fishery (Section II). We explored alternative candidate models that accounted for movement in and out of Chatham Strait and incorporated fishery CPUE. Finally, because of the substantial increase in estimated abundance since 2016, coupled with uncertainty in recruitment, we adjusted the point estimate for the `r YEAR` exploited abundance by taking the 15th percentile from the posterior distribution of abundance before conducting the yield per recruit analysis and calculation of $ABC$ (`r fig('N_posterior', display = "cite")`). This adjustment serves to stabilize the fishery and is intended to guard against overfishing if the 2014 year class is lower than currently predicted. Moreover, because fish younger than 7 years comprise 15-20% of the forecasted exploited biomass in 2018 and less than 50% of females are mature at this age, this adjustment will help protect future spawning potential for this year class. 
...
`r tbl('tag_sum', caption = paste0('A summary of data inputs to the mark-recapture models, including total individuals marked ($K_0$), marks not available to the longline survey or the fishery (captured in other fisheries or outside Chatham; $D_0$), recaptured individuals in the longline survey and fishery ($k_{srv}$ and $k_{fsh}$), number of sampled individuals in the longline survey and fishery ($n_{srv}$ and $n_{fsh}$), marks not available to the fishery that were captured outside Chatham or in other fisheries during the survey ($D_{srv}$), and marks caught in other fisheries or outside Chatham during the fishery ($D_{fsh}$) for years with a mark survey from 2005 to ', YEAR ,'. '))`
