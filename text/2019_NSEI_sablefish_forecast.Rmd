...
diff <- formatC(abs(summary_tbl_raw[9,3] - summary_tbl_raw[9,2]), format = "d", big.mark = ",")
diff_notadj <- formatC(abs(summary_tbl_raw[8,3] - summary_tbl_raw[8,2]), format = "d", big.mark = ",")
...
# abundance last year as estimated this year and adjusted for uncertaintly in recruitment
abd_lyadj <- summary_tbl_raw[3,2]$Y2018 / 1e6

# abundance last year as forecasted from the previous year and used for mgmt in YEAR
abd_ly <- summary_tbl_raw[1,2]$Y2018 / 1e6

# forecasted abundance recommended for management in current year
abd_foradj <- summary_tbl_raw[1,3]$Y2019 / 1e6

eval_abd <- ifelse(abd_lyadj > abd_ly, "an increase from the ",
                   ifelse(abd_lyadj == abd_ly, "the same as the ", "a decrease from the "))
                      
# format these values for text
abd_lyadj <- formatC(abd_lyadj, format="f", digits = 2)
abd_ly <- formatC(abd_ly, format="f", digits = 2)
abd_foradj <- formatC(abd_foradj, format="f", digits = 2)
...
short_sum <- summary_tbl[c(1, 4, 7, 9),]
...
- The `r YEAR+1` recommended Allowable Biological Catch ($ABC$) for Northern Southeast Inside Waters (NSEI - Chatham Strait) at a fully-selected fishing mortality of $F_{50\%}$ = `r ABC_adj` round lb (`r tbl('full_summary', display = "cite")`). This is a `r diff` lb increase (`r perc_change`%) from the `r YEAR` $ABC$ of `r ABC_ly` round lb.

- An adjustment to the $ABC$ is recommended to account for continued uncertainty in the size of 2013 and 2014 year classes. This adjustment was made by using the 15th percentile from the posterior distribution of the mark-recapture abundance estimate instead of the mean as the input to the forecast model (`r (fig('N_posterior', display = "cite")`). The 2014 year class is estimated to be the largest recruitment event for sablefish since 1977 (Hanselman et al. `r YEAR`). If the `r YEAR` $ABC$ is not adjusted, the `r YEAR+1` $ABC$ would be `r ABC_notadj` lb, a `r diff_notadj` lb increase (`r perc_change_notadj`%) from `r YEAR` (`r tbl('full_summary', display = "cite")`). Because fish younger than 7 years comprise approximately 20% of the forecasted exploited biomass for `r YEAR+1` and less than 50% of females are estimated to be mature at this age, this adjustment will help protect future spawning potential for these year classes (`r fig('forec_Natage', display="cite")`).

- The recommended population estimate from the `r YEAR` mark-recapture study in Chatham Strait is `r abd_lyadj` individuals (`r tbl('full_summary', display = "cite")`). This is `r eval_abd` forecasted `r YEAR-1` mark-recapture estimate of `r abd_ly` sablefish. The forecast abundance for `r YEAR+1` is `r abd_foradj` individuals (`r fig('retro', display="cite")`; `r tbl('full_summary', display = "cite")`). Note that the forecasted abundance will always be lower than the current year's estimate of abundance under the current management model.
...
3.  A preliminary age-structured model is presented as an Appendix to this report. Once reviewed by managers and stakeholders, it will be presented as an alternative to the current management model.
...
`r fig('fsh_cpue', caption = paste0('Commercial longline fishery CPUE in round lb per hook (+/- 1 standard deviation), 1997-', YEAR ,'. '))`
