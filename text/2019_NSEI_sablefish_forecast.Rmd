...
Fishing mortality $F$ was obtained from a yield-per-recruit analysis and fixed to $F=F_{50}$, where $F_{50}$ corresponds to the $F$ that would reduce the spawning biomass to 50% of the unfished biomass. Biological inputs to the yield-per-recruit model included longline survey weight-at-age and estimated maturity from the longline survey (`r fig('bio_inputs', display = "cite")`). An $ABC$ of `r ABC` round lb was calculated as the landed portion of the total catch under $F_{50}$. The discarded portion of the catch that is assumed to die given a 16% discard mortality rate was calculated as `r discards` round lb (Section IV, `r tbl('full_summary', display = "cite")`). 
...
Using longline survey weight-at-age by sex $w_{s,a}$ from a weight-based von Bertalanffy growth model, a modified Baranov catch equation is used to calculate the acceptable biological catch in `r YEAR+1` $ABC_{`r YEAR+1`}$:
...
The $ABC$ for `r YEAR+1` was estimated to be `r ABC` round lb, and the discarded portion of the catch was estimated to be `r discards` round lb (`r tbl('full_summary', display = "cite")`). `r fig('ABC_ts', display = "cite")` shows the `r YEAR+1` $ABC$ in the context of $ABC$s recommended by ADF&G since 2005.
...
`r fig('ABC_ts', caption = paste0('Acceptable biological catch $ABC$ in millions of round lb recommended by ADF&G, 2005-', YEAR+1, '. The grey dashed vertical line marks a change in harvest policy from $F_{40}$ before 2009, $F_{45}$ in 2009, and $F_{50}$ in 2010 to present.'))`
