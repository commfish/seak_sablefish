...
Ultimately, Model 1 with $P=6$ was selected for the `r YEAR+1` forecast. Retrospective analysis shows Model 1 abundance estimates follow a similar trend and general magnitude as past model estimates (`r fig('retro', display = "cite")`). As discussed in Section I, the 15th percentile from the posterior distribution was used to adjust the abundance estimate for subsequent analyses (`r fig('N_posterior', display="cite")`).

# Section III: Determining Allowable Biological Catch

Using the adjusted abundance estimate in `r YEAR` ($N_{`r YEAR`}$ in the equation below), the vulnerable abundance-at-age for ages 2+ in `r YEAR+1` was obtained by partitioning the estimated exploited abundance into cohorts using commercial fishery age compositions and applying fishing and natural mortality:
...
Total instantaneous mortality-at-age $Z_{s,a}$ is the sum of fishing mortality $F_{s,a}$ and natural mortality $M$, which is assumed to be 0.10 (Johnson and Quinn, 1998). Fully-selected fishing mortality $F$ is obtained from yield-per-recruit tables, and fixed to $F=F_{50}$, where $F_{50}$ corresponds to the $F$ that would reduce the spawning biomass to 50% of the unfished biomass. Biological inputs to the yield-per-recruit model included longline survey weight-at-age and estimated maturity from the longline survey (`r fig('bio_inputs', display = "cite")`).
...
Discard mortality is assumed to be 0.16, the discard mortality used in the Pacific halibut fishery (Gilroy and Stewart 2013). Pacific halibut are a reasonable proxy for sablefish because they are large-bodied, long-lived benthic fish that do not experience barotrauma. Retention probability by sex and age $R_{a,s}$ is informed by processor grade and price and defined as a function of weight, which is converted to age and sex using survey weight-at-age (`r fig('discard_mort', display = "cite")`). This method of accounting for discards shifts fishing mortality toward older ages, especially for males that are slower growing than females (`r fig('F_discards', display = "cite")`).

Using longline survey weight-at-age by sex $w_{s,a}$ from a weight-based von Bertalanffy growth model, a modified Baranov catch equation is used to calculate the expected landed biomass in `r YEAR+1` $L_{`r YEAR+1`}$ :

$$L_{`r YEAR+1`}= \sum_{s=1}^{2}\sum_{a=2}^{a+}w_{s,a}(N_{`r YEAR+1`,s,a})\frac{R_{s,a}S_{s,a}F}{Z_{s,a}}(1-\mbox{exp}(-Z_{s,a})).$$

Similarly, the biomass of discarded sablefish estimated to die with an assumed discard mortality of 0.16 $D_{`r YEAR+1`}$ is

$$D_{`r YEAR+1`}= \sum_{s=1}^{2}\sum_{a=2}^{a+}w_{s,a}(N_{`r YEAR+1`,s,a})\frac{\Omega S_{s,a}(1-R_{s,a})F}{Z_{s,a}}(1-\mbox{exp}(-Z_{s,a})).$$

The `r YEAR+1` $ABC$ is calculated as the difference between the landed catch and the amount of fish dying due to discard, $ABC_{`r YEAR+1`} = L_{`r YEAR+1`} - D_{`r YEAR+1`}$. The landed and discarded portions of the catch were estimated to be 1,140,009 and 20,672 round lb, with a resulting $ABC$ of 1,119,337 round lb.
...
`r fig('retro', caption = paste0("Abundance estimates from the current model that have been adjusted for uncertainty in recruiment (black points), current estimates before the adjustment (black triangles), and previous estimates of abundance that were used for management in a given year (grey squares) from 2005-", YEAR ,". Shaded areas are 95% credible intervals from the current estimates' posterior distributions prior to adjustments. The grey square in ", YEAR ," is the forecasted abundance from ", YEAR-1 ,"."))`
