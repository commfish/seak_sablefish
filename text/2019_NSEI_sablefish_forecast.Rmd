...
summary_tbl %>% replace_na(list(`2018` = " ", `Percent change (%)` = " "))

replace(summary_tbl$`Percent change (%)`, "NA", "")
...
- An adjustment to the $ABC$ is recommended to account for continued uncertainty in the size of 2013 and 2014 year classes. This adjustment was made by using the 15th percentile from the posterior distribution of the mark-recapture abundance estimate instead of the mean as the input to the forecast model (`r fig('N_posterior', display = "cite")`). The 2014 year class is estimated to be the largest recruitment event for sablefish since 1977 (Hanselman et al. `r YEAR`). If the `r YEAR` $ABC$ is not adjusted, the `r YEAR+1` $ABC$ would be `r ABC_notadj` lb, a `r diff_notadj` lb increase (`r perc_change_notadj`%) from `r YEAR` (`r tbl('full_summary', display = "cite")`). Because fish younger than 7 years comprise approximately 20% of the forecasted exploited abundance for `r YEAR+1` and less than 50% of females are estimated to be mature at this age, this adjustment will help protect future spawning potential for these year classes (`r fig('forec_Natage', display="cite")`).
...
##### Posterior distribution of abundance and recommended value for forecast
