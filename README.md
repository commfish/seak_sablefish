...
Fisheries-independent data and inputs to the `TMB` model are made available under [`data/`](/blob/master/data) of this repository. These include biological data and indices of effort. Fisheries-dependent data are not made public to protect fishermen and processor confidentiality, but may be obtained through a formal data request to ADF&G. All `SQL` queries used to obtain data are found in [`r/0_querynclean_data.r`](/blob/master/r/0_querynclean_data.R).
