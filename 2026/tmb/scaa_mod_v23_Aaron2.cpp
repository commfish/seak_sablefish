...
Type objective_function<Type>::operator() ()
{
  ...
  //switch for extra variance for indices
  DATA_INTEGER(ev_type)
  ...
  array<Type> F_base(nyr, nage, nsex); // Aarons attempt to fix catch accounting; This will be Fmort and Selex
  ...
  //   }
  // }
  // Aaron 2026: v23a fix for landed catch
  for (int k = 0; k < nsex; k++) {
    for (int i = 0; i < nyr; i++) {
      for (int j = 0; j < nage; j++) {
        
        // Base fishing mortality (selectivity applied, no retention yet)
        F_base(i,j,k) = Fmort(i) * fsh_slx(i,j,k);
        
        // Total mortality includes both retained and discard mortality components
        Z(i,j,k) = M + F_base(i,j,k) * (retention(0,j,k) + dmr(i,j,k) * (Type(1.0) - retention(0,j,k)));
        
        // Survivorship
        S(i,j,k) = exp(Type(-1.0) * Z(i,j,k));
        
        // Store F_base in the F array for later use (or you could eliminate F array entirely)
        F(i,j,k) = F_base(i,j,k);
      }
    }
  }
  ...
  //   }
  // }
  // Aaron fix to be consistent with above
  for (int k = 0; k < nsex; k++) {
    for (int i = 0; i < nyr; i++) {
      for (int j = 0; j < nage; j++) {
        survival_srv(i,j,k) = exp(Type(-1.0) * srv_month * Z(i,j,k));
        survival_fsh(i,j,k) = exp(Type(-1.0) * fsh_month *  Z(i,j,k));
        survival_spawn(i,j,k) = exp(Type(-1.0) * spawn_month * Z(i,j,k));
      }
    }
  }
  ...
  // Aaron attempt to fix this to be consistent with how F is calculated above
  // Need to add discard mortality back in here..
  for(int x = 0; x <= n_Fxx; x++) {
    Nspr(x,0) = Type(1.0);
    
    for(int j = 1; j < nage - 1; j++) {
      Type F_total = Fxx(x) * spr_fsh_slx(j-1) * (retention(0,j-1,nsex-1) + dmr(nyr-1,j-1,nsex-1) * (Type(1.0) - retention(0,j-1,nsex-1)));
      Nspr(x,j) = Nspr(x,j-1) * exp(Type(-1.0) * (F_total + M));
    }
    
    Type F_total_minus1 = Fxx(x) * spr_fsh_slx(nage-2) * (retention(0,nage-2,nsex-1) + dmr(nyr-1,nage-2,nsex-1) * (Type(1.0) - retention(0,nage-2,nsex-1)));
    Type F_total_plus = Fxx(x) * spr_fsh_slx(nage-1) * (retention(0,nage-1,nsex-1) + dmr(nyr-1,nage-1,nsex-1) * (Type(1.0) - retention(0,nage-1,nsex-1)));
    
    Nspr(x,nage-1) = Nspr(x,nage-2) * exp(Type(-1.0) * (F_total_minus1 + M)) /
  ...
  for(int j = 0; j < nage; j++) {
    ...
    SBPR(0) += Nspr(0,j) * prop_mature(0,j) * data_srv_waa(0,j,nsex-1) * exp(Type(-1.0) * spawn_month * M); // Aaron Changed so no F
  }
  ...
  //   }
  // }
  // Aaron fixed code for new Fxx calculation:
  for(int x = 0; x < n_Fxx; x++) {
    for(int k = 0; k < nsex; k++) {
      for(int j = 0; j < nage; j++) {
        // Base F at Fxx level (before retention/discard split)
        Type F_base_xx = Fxx(x+1) * fsh_slx(nyr-1,j,k);
        
        // Total mortality includes both retained and discard components
        Z_Fxx(x,j,k) = M + F_base_xx * (retention(0,j,k) + dmr(nyr-1,j,k) * (Type(1.0) - retention(0,j,k)));
        S_Fxx(x,j,k) = exp(-Z_Fxx(x,j,k));
        
        // Store base F for use in ABC calculation
        sel_Fxx(x,j,k) = F_base_xx;
      }
    }
  }
  ...
  // Indices likelihoods... case 0 = pre-weighted, case 1 = extra variance estimated... 
  
  switch(ev_type) {
  case 0:// no extra variance
  ...
  case 1: // extra variance and removed fixed weighting
  ...
  if (random_rec == 0) {
    ...
    for (int i = 0; i < nyr; i++) {
      rec_like += square(log_rec_devs(i) - Type(0.5) * square(sigma_r))/square(sigma_r);
    }
    
    // Initial numbers-at-age (sage + 1 to plus group - 1)
    for (int j = 0; j < nage - 2; j++) {
      rec_like += square(log_rinit_devs(j) - Type(0.5) * square(sigma_r))/square(sigma_r);
    }
  ...
  ADREPORT(tot_expl_abd);     // Vulnerable abundance to fishery at the beginning of the fishery
  ...
  ADREPORT(tot_spawn_biom);   // Female spawning biomass
