...
ll_cpue %>% mutate(no_hooks_p_set = set_hook_count_best_available) %>%
...
# Trends over time by area #DEPRECATED 2024 BC STAT AREA NO LONGER USED
...
fsh_cpue_cl %>% 
  select(Gear, hook_size, set_depth, soak_p_set, #Adfg, 
         julian_day_sell, set_length, start_lon, start_lat) %>% #cardinality_threshold=NULL
...
cpue_exam <- fsh_cpue_cl %>% select(Year,cpue,std_cpue,Gear,hook_size,set_depth,
                                    soak_p_set,Adfg,start_lat,start_lon,julian_day_sell,
                                    set_length,dum) %>%
...
m0.lat <- bam(cpue ~ Year + Gear + s(start_lat), data=cpue_exam, gamma=1.4)
m0.lon <- bam(cpue ~ Year + Gear + s(start_lon), data=cpue_exam, gamma=1.4)
...
global<-bam(cpue ~ Year + Gear + hook_size + s(set_depth, k=4) + s(soak_p_set, k=4) + 
              s(Adfg, bs='re', by=dum) + te(start_lon, start_lat) +
...
# Determine if random variables should be included (Stat and Adfg)
# Stat area commented out for now but here if this changes in 2025. 
...
m2 <- bam(cpue ~ Year + Gear + hook_size + s(set_depth, k=4) + s(soak_p_set, k=4) + 
            s(Adfg, bs='re', by=dum), data=cpue_exam, gamma=1.4)
m3 <- bam(cpue ~ Year + Gear + hook_size + s(set_depth, k=4) + s(soak_p_set, k=4),
          data=cpue_exam, gamma=1.4)
m4 <- bam(cpue ~ Year + Gear + hook_size + s(set_depth, k=4) + s(soak_p_set, k=4), 
          data=cpue_exam, gamma=1.4)
...
m5 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set, k=4) +    #no hook size
            te(start_lon, start_lat)+ s(Adfg, bs='re', by=dum), data=cpue_exam, gamma=1.4)
m6 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set, k=4) +    #hook size as a random variable
            s(hook_size, bs='re', by=dum) + te(start_lon, start_lat)+ 
            s(Adfg, bs='re', by=dum), data=cpue_exam, gamma=1.4)
...
m7 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set, k=4) + 
            ...
            s(Adfg, bs='re', by=dum) + te(start_lon, start_lat), data=cpue_exam, gamma=1.4)
m8 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set) + 
            hook_size + #s(Hook_size, bs='re', by=dum) + 
            s(Adfg, bs='re', by=dum) + s(start_lon), data=cpue_exam, gamma=1.4)
m9 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set, k=4) + 
            hook_size + #s(Hook_size, bs='re', by=dum) + 
            s(Adfg, bs='re', by=dum) + s(start_lat), data=cpue_exam, gamma=1.4)
...
m10 <- bam(cpue ~ Year + Gear + s(set_depth, k=4) + s(soak_p_set, k=4) + 
             hook_size + #s(Hook_size, bs='re', by=dum) +  
             s(Adfg, bs='re', by=dum) + s(start_lat, k=6), data=cpue_exam, gamma=1.4)
AIC(global,m1, m6, m7, m8, m9, m10)
...
m11 <- bam(cpue ~ Year + Gear + s(julian_day_sell, k=4) + s(set_depth, k=4) + 
             ...
             hook_size + te(start_lon, start_lat) + 
             s(Adfg, bs='re', by=dum), data=cpue_exam, gamma=1.4)
AIC(global,m1,m6,m7,m9, m11) #AIC(m1, m7, m9, m11)
BIC(global,m1,m6,m7,m9, m11) #AIC(m1, m7, m9, m11)
summary(m11)
...
AIC(global, m1,m2,m3,m4,m5,m6,m7,m11)
...
std_dat <- expand.grid(year = unique(cpue_exam$Year),
                       ...
                       start_lat = mean(cpue_exam$start_lat),
                       start_lon = mean(cpue_exam$start_lon),
...
# compared to last year's standardization: 
fsh_sum %>%
  select(year, cpue = fsh_cpue, upper, lower) %>% 
  mutate(CPUE = "Nominal") %>% 
  bind_rows(std_dat %>% 
