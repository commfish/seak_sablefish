...
read.csv(paste0(YEAR+1,"/data/fishery/raw_data/nseiharvest_ifdb_",
  ...
  select(year = YEAR, date, julian_day, Mgmt_area = G_MANAGEMENT_AREA_CODE, Stat = G_STAT_AREA,
         ...
         #Cfec_permit = G_CFEC_FISHERY,
         Cfec_permit = CFEC_FISHERY, #AGR changed
         Delivery_cde = DELIVERY_CODE, 
...
read_csv(paste0(YEAR+1,"/data/survey/raw_data/llsrv_cpue_1985_",
         ...
         guess_max = 50000) %>% 
  filter(YEAR <= YEAR) %>%
  mutate(date = as.Date(format(parse_date_time(TIME_SECOND_ANCHOR_OVERBOARD, c("%m-%d-%Y %H:%M")),"%Y-%m-%d")), #ISO 8601 format #AGR update - dae
         julian_day = yday(date),
         time1 = mdy_hm(TIME_FIRST_ANCHOR_ONBOARD), #AGR update - 
         time2 = mdy_hm(TIME_SECOND_ANCHOR_OVERBOARD), #AGR update
...
read_csv(paste0(YEAR+1,"/data/survey/raw_data/llsrv_by_condition_1988_", YEAR, ".csv"), 
         guess_max = 50000) %>% 
  filter(YEAR <= YEAR) %>%
  mutate(date = as.Date(format(parse_date_time(TIME_FIRST_BUOY_ONBOARD, c("%m-%d-%Y %H:%M")),"%Y-%m-%d")), #agr changed this
...
read_csv(paste0(YEAR+1,"/data/survey/raw_data/llsrv_bio_", 
                YEAR, ".csv"), 
         guess_max = 50000) %>% 
  mutate(date = date(parse_date_time(TIME_FIRST_BUOY_ONBOARD, c("%m-%d-%Y %H:%M"))), #AGR add
         #mutate(date = date(parse_date_time(TIME_FIRST_BUOY_ONBOARD, c("%Y-%m-%d %H:%M:%S"))), #ISO 8601 format AGR deactivated
...
read_csv(paste0(YEAR+1,"/data/survey/raw_data/potsrv_bio_", YEAR, ".csv"), 
  ...
  mutate(date = mdy(as.Date(TIME_FIRST_BUOY_ONBOARD)), #ISO 8601 format #agr adjusted so the date works
         julian_day = yday(date),
         Sex = derivedFactor("Male" = SEX_CODE == "01",
                             "Female" = SEX_CODE == "02",
                             .default = NA),
...
read_csv(paste0(YEAR+1,"/data/survey/raw_data/tag_releases_",
 ...
 # mutate(date = date(parse_date_time(time_second_anchor_onboard)),
  mutate(date = mdy_hm(time_second_anchor_onboard),
    #julian_day = lubridate::yday(date),
...
tag_recoveries<-read.csv(paste0(YEAR+1,"/data/fishery/raw_data/tag_recoveries_",
                     YEAR, ".csv")) %>%   
  mutate(landing_date = mdy_hm(LANDING_DATE), #, tz="America/Anchorage"),
...
# 2025 DO WE NEED THIS? SPENCER DOESN'T KNOW WHERE TO FIND THIS
