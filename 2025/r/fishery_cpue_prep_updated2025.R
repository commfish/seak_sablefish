# Fishery CPUE Data Prep
# This script is for #2 in the 0_clean_data.R file and preps data for use in fishery_ll_cpue.R 
# amd fishery_pot_cpue.R.
...
#FLAG!!!!! We have omitted stat area from the 2024 analysis but I (Phil) think it
...
#                          "335701", "345701", "345731", "345803")) %>% # remove survey harvest fish tickets
  select(year, trip_number, project_code, ft_adfg_number = adfg_number, vessel_name, elandings_trip_num, ticket_number, cfec_fishery_code, 
         ...
         ft_species = species, ft_rnd_lbs = round_pounds) #ft_stat = stat_area NOT A COLUMN IN 2025 DATA FILE ## do not use stat_area from fish ticket data -- it's incomplete and wrong! 
...
pot_log <- pot_log %>% mutate(pot_dim_raw1 = pot_dimensions_1,
  ...
  mutate(pot_volume_m3 = ifelse(pot_type_1 == "Slinky",
                                 0.00001639*pi*pot_len*((0.5*pot_diam)^2),
                                 0.00001639*(pi*(sqrt(pot_len1*pot_len1-(pot_diam2-pot_diam1)^2))/3 * ((0.5*pot_diam2)^2+0.5*pot_diam2*pot_diam1+(0.5*pot_diam1)^2))),
         pot_len = ifelse(pot_type_1 == "Slinky",pot_len,pot_len1),
...
pot_cpue_log_ftx <- pot_log %>% 
  ...
  mutate(#across(where(is.character),as.factor),
         time_set = parse_date_time(time_set, c("%m/%d/%Y %H:%M")), #as.Date(sell_date, format = "%m/%d/%Y")
         time_hauled = parse_date_time(time_hauled, c("%m/%d/%Y %H:%M")),
         pot_line_diam_1 = as.numeric(pot_line_diam_1),
         pot_line_diam_2 = as.numeric(pot_line_diam_2),
         pot_spacing_1 = as.numeric(pot_spacing_1),
         pot_spacing_2 = as.numeric(pot_spacing_2),
         num_of_pots_set_1 = as.numeric(num_of_pots_set_1),
         num_of_pots_set_2 = as.numeric(num_of_pots_set_2),
         num_of_pots_lost_1 = as.numeric(num_of_pots_lost_1),
         num_of_pots_lost_2 = as.numeric(num_of_pots_lost_2),
         trip_recorded_releases = case_when(!is.na(log_numbers_sablefish_released) | !is.na(log_numbers_sablefish_released_quota_limit) | 
                                              !is.na(log_rnd_lbs_sablefish_released) | !is.na(log_rnd_lbs_sablefish_released_quota_limit) ~ "some_released", 
...
pot_cpue_log_ftx %>% filter(effort_target_species_code_1 == 710) %>% 
  ...
  mutate(trip_soak_time = sum(soak_time_hrs),
         ...
         pots_p_set = num_of_pots_set_1-num_of_pots_lost_1, #trip_pots / set_count,
         mean_trip_depth_fm = mean(average_depth_fathoms),
         #trip_hooks_exact = sum(set_hook_count_exact),
         #trip_hooks_est = sum(set_hook_count_best_available),
         trip_sable_lbs_set = sum(unique(trip_catch))/set_count, #otherwise, count it using the number of sets that recorded a landing... 
         #sable_lbs_set = sum(unique(trip_catch))/set_count,
         lbs_p_pot = sum(unique(trip_catch))/(num_of_pots_set_1-num_of_pots_lost_1),
         #lbs_p_hk_all = sum(unique(trip_catch))/trip_hooks_est,
         #multigear_trip = ifelse(multi_gear_config == "single_config","No","Yes"),
         skip_acc = ifelse(!is.na(log_rnd_lbs_sablefish_retained),
                           (sum(log_rnd_lbs_sablefish_retained )/ unique(trip_catch)),NA),#stat_area_match = ifelse(ft_stat == log_stat_area,"Match","Mismatch"),
...
pot_cpue %>%
  mutate(set_date = as.Date(time_set,c("%Y-%m-%d %H:%M:%S"), tz="UTC" ),
         ...
         line_diam_1 = 2.54*pot_line_diam_1, # (convert to cm)
         pot_dim_1 = factor(pot_dimensions_1),
         pot_type_1 = factor(pot_type_1),
         pot_space_2 = 0.3048*pot_spacing_2,  #(convert to meters)
         line_diam_2 = 2.54*pot_line_diam_2, # (convert to cm)
  ...
  select(year,set_date, sell_date = ft_sell_date, julian_day_set, julian_day_sell,
         trip_no = trip_number,
         Adfg = adfg_number,
         tm_hauled = time_hauled, t_set = time_set, date= ft_sell_date, 
         set_target = effort_target_species_1,
         trip_target = trip_target_species_code_1,
         start_lat = start_latitude_decimal_degrees,
         start_lon = start_longitude_decimal_degrees,
         set_soak = soak_time_hrs,
         set_length = set_length_km, set_depth_m, avg_trip_depth_m = trip_depth,
         gear_name, pot_space_1,
         line_diam_1, pot_dim_1,pot_type_1,
         pot_space_2, line_diam_2, 
         pot_dim_2,pot_type_2,
         trip_pots, pots_p_set,
         set_depth = set_depth_m,trip_depth,
         sable_lbs_set, sable_lbs_pot, set_no = effort_number,
         no_sets = set_count, trip_set_targets = trip_target_species_code_1,
         trip_recorded_releases,
         disposition = ft_dispo_code,
         catch = set_catch,
         sable_lbs_set, sable_lbs_pot
         ) -> pot_eff

#save for 2025 assessment...
write_csv(pot_eff, paste0(YEAR+1,"/data/fishery/fishery_pot_cpue_",
                          min(pot_eff$year), "_", max(pot_eff$year), ".csv"))

#save for legacy for 2026 and forward
